          ...
          <div key={challenge.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 text-xs text-[#CEF17B]">{challenge.progressText}</p>
              </div>
              <Button onClick={() => onCompleteToday(challenge)} disabled={isSaving || challenge.completedToday || challenge.status === 'completed'} className="h-10 rounded-2xl bg-[#CEF17B] px-4 text-xs font-bold text-[#0B3936] hover:bg-[#bfe56b]">
                {challenge.completedToday ? 'Feito hoje' : challenge.status === 'completed' ? 'Concluído' : 'Concluir hoje'}
              </Button>
            </div>
