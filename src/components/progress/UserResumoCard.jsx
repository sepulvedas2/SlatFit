...
export default function UserResumoCard({ xp, level, ranking, streak, onOpenRanking }) {
  return (
    <Card className="rounded-3xl border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-black/10">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <button onClick={onOpenRanking} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left transition-all hover:bg-white/[0.06]"><p className="text-xs text-white/40">Ranking</p><p className="mt-2 text-2xl font-black text-white">{ranking ? `#${ranking}` : '—'}</p></button>
