...
const COLORS = [
  { label: "Lime", value: "lime", hex: "#CEF17B" },
  { label: "Orange", value: "orange", hex: "#FF6A00" },
...
export default function HabitCreateModal({ open, onClose, onSubmit, isLoading }) {
      ...
      >
        ...
        >
          <div className="mb-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/35">New Habit</p>
          ...
          <div className="space-y-4">
            <div>
              <Label className="text-white/60">Habit name</Label>
              <Input value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} className="mt-2 border-white/10 bg-white/5 text-white" placeholder="Ex: Ler 20 páginas" />
            </div>
            ...
            <div>
              <Label className="text-white/60">Target frequency (days/week)</Label>
            ...
            <div>
              <Label className="text-white/60">Preferred time of day</Label>
              <div className="mt-2 grid grid-cols-3 gap-2">
                {TIMES.map((time) => (
                  <button
                    key={time.value}
